 ...
 * @author mtereshchuk
 */
class CreateAccountServlet(private val managerAdmin: ManagerAdmin) : AbstractServlet() {
...
class ExtendAccountServlet(private val managerAdmin: ManagerAdmin) : AbstractServlet() {
    override fun baseDoGet(request: HttpServletRequest, response: HttpServletResponse) {
        val id = request.getParameter("id")?.toInt() ?: error("Missing 'id'")
...
class GetInfoServlet(private val managerAdmin: ManagerAdmin) : AbstractServlet() {
    override fun baseDoGet(request: HttpServletRequest, response: HttpServletResponse) {
        val id = request.getParameter("id")?.toInt() ?: error("Missing 'id'")
